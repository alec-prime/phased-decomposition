...
Residual ambiguity through phases 0–5 is routed through the LLM collaborator, which asks questions in the operator's native register and renders the answers into production vocabulary on their behalf. The operator never has to shift register to commit execution-grade decisions. Phase 6 runs without the operator against commitments already locked upstream; UAT is the return point.

Each phase has a primary mode. Phase 0 teaches — its artifacts orient a reader to the project from cold. Phase 1 illustrates — it makes the concept tangible and maps the route to completion. Phase 2 commits — it turns a planned release into an active project. Later phases narrow into per-release architecture and execution. The progression mirrors how understanding actually deepens: from why, to what, to how, to do.

### The Phases

The pipeline is seven phases. Each produces a terminal artifact (or set of artifacts) that carries the phase's decisions and rationale forward.

- **Phase 0 — Concept.** Articulates the project from cold. The concept doc teaches a cold reader the project's kernel, stakes, intent, shape, and precedent. The kernel — a compressed statement of what the project is, one layer below its surface framing — is the load-bearing element every downstream phase descends from, and surfacing it is Phase 0's most consequential production requirement. Non-technical, legible to outside readers. _Artifacts:_ `concept.md`.

- **Phase 1 — Northstar, Mockup & Roadmap.** Makes the concept tangible and maps the route to completion. The [[NORTHSTAR|northstar]] describes the scenes that illustrate the concept's payoff at the level of abstraction — re-instantiable, with no specific operator, project, or filenames. The [[MOCKUP|mockup]] is one concrete instantiation of those scenes in the medium the project's external surface calls for. The [[ROADMAP|roadmap]] maps the route from the illustrated end state to a sequence of independent releases; for single-release projects the roadmap is minimal but structurally present. The first outward-communicating artifacts; a guide for downstream phases, not a lock. _Artifacts:_ `northstar.md`, `mockup.md`, `roadmap.md`.

- **Phase 2 — Release Activation.** Per-release. Commits the artifacts that turn a planned release into an active project. The release artifact commits what the release ships, the binding quality standard, the executor roster with reference documentation, scope exclusions, and the definition of done. Sibling activation artifacts bind the release to its validation gate, its calendar, and its operational session-start instructions. Subagent definition files in `.claude/agents/` commit the full executor definitions. For projects with multiple releases, files are scoped per release (e.g., `release_R1.md`); single-release projects drop the suffix. _Artifacts:_ `release.md` (or `release_R{N}.md`), `test-plan.md`, `schedule.md`, `.claude/agents/` subagent definitions, `CLAUDE.md`.
...
- **Phase 5 — Execution Planning.** Per-release. Decomposes the spec into a tree whose leaves are sized per the executors the release's subagent definitions committed, and authors the evaluation set the execution loop will be gated against at every level. Produces dual artifacts from one underlying tree: a human-readable plan summarized for operator review, and a machine-readable plan carrying the full decomposition for Phase 6. Terminates at a review gate covering plan and evaluation set together. _Artifacts:_ human-readable plan, machine-readable plan, evaluation set.

- **Phase 6 — Execution.** Per-release. The executors named in the release's subagent definitions run the leaves from Phase 5's machine-readable plan; the supervisor gates each against Phase 5's evaluation set, halting on failure and routing for rework or escalation. No planning or derivation happens here. _Artifacts:_ completed leaves, passing evaluations.

Phases 0–1 are framing work shared by the project as a whole. Phase 2 commits per release. Phases 3–6 repeat per release, with each release treated as an independent, complete product rather than an increment of a single system.
...
- **2026-04-16 — Roadmap relocated from Phase 0 to Phase 1; Phase 2 mode relabeled.** Roadmap moved from Phase 0 (Concept & Roadmap) to Phase 1 (Northstar, Mockup & Roadmap). Phase 0 narrowed to concept only — create the idea. Phase 1 broadened to illustrate the idea and map the route to completion. Phase 2 mode relabeled from implicit "activates" to "commits." Pipeline count corrected from five to seven. Progression sequence updated: concept, illustration, commitment, architecture, execution. _Why:_ the three upstream modes — create, visualize, map — sort naturally into two phases: Phase 0 creates (concept), Phase 1 visualizes and maps (northstar + mockup + roadmap). Roadmap was structurally closer to illustration than to concept; both northstar/mockup and roadmap are outward-communicating artifacts that make the concept tangible and navigable, while concept stands alone as the teaching artifact. _Affects:_ Phase 0, Phase 1, Phase 2 bullets in §The Phases; mode paragraph in §The Thesis; Introduction progression sequence; framing summary paragraph. Downstream: `ROADMAP.md` frontmatter, `ARCHITECTURE.md` skill directory, `SPEC.md` reference file specs and per-phase role application table, `SPEC.md Amendment.md` reference file specs, `project-notes.md` framework-level artifact list.