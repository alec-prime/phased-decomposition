...
- **Phase 0 — Concept & Roadmap.** Articulates the project from cold and maps the route to completion. The concept doc teaches a cold reader the project's kernel, stakes, intent, shape, and precedent. The kernel — a compressed statement of what the project is, one layer below its surface framing — is the load-bearing element every downstream phase descends from, and surfacing it is Phase 0's most consequential production requirement. The roadmap maps the route from the illustrated end state to a sequence of independent releases; for single-release projects the roadmap is minimal but structurally present. Non-technical, legible to outside readers. _Artifacts:_ `concept.md`, `roadmap.md`.

- **Phase 1 — Northstar & Mockup.** Makes the concept tangible. The [[NORTHSTAR|northstar]] describes the scenes that illustrate the concept's payoff at the level of abstraction — re-instantiable, with no specific operator, project, or filenames. The [[MOCKUP|mockup]] is one concrete instantiation of those scenes in the medium the project's external surface calls for. The first outward-communicating artifacts; a guide for downstream phases, not a lock. _Artifacts:_ `northstar.md`, `mockup.md`.

- **Phase 2 — Release Activation.** Per-release. Activates a single release by committing the artifacts that turn it from a planned unit on the roadmap into an active project. The release artifact commits what the release ships, the binding quality standard, the executor roster with reference documentation, scope exclusions, and the definition of done. Sibling activation artifacts bind the release to its validation gate, its calendar, and its operational session-start instructions. Subagent definition files in `.claude/agents/` commit the full executor definitions. For projects with multiple releases, files are scoped per release (e.g., `release_R1.md`); single-release projects drop the suffix. _Artifacts:_ `release.md` (or `release_R{N}.md`), `test-plan.md`, `schedule.md`, `.claude/agents/` subagent definitions, `CLAUDE.md`.
...
Phases 0–1 are framing work shared by the project as a whole. Phase 2 activates per release. Phases 3–6 repeat per release, with each release treated as an independent, complete product rather than an increment of a single system.
